...
extern const int mapCubeArrayLength =
    mapCubeLength / mapResolution;  // the grid dimension of a sub-map
                                    // (mapCubeLength / mapResolution)
extern const int mapArrayLength =
    2000 /
    mapCubeLength;  // the sub-map dimension of global map (2000m x 2000m)
extern const int rootCubeIndex =
    mapArrayLength /
    2;  // by default, robot is at the center of global map at the beginning
...
extern const float filterHeightLimit =
    (urbanMapping == true) ? 0.1 : 0.15;   // step diff threshold
extern const float filterAngleLimit = 20;  // slope angle threshold
extern const int filterHeightMapArrayLength =
    sensorRangeLimit * 2 / mapResolution;

// BGK Prediction Params
extern const bool predictionEnableFlag = true;
extern const float predictionKernalSize =
    0.2;  // predict elevation within x meters
...
extern const int localMapLength =
...
extern const float visualizationFrequency =
    2;  // n, skip n scans then publish, n=0, visualize at each scan
...
extern const std::vector<int> costHierarchy(
    tmp,
    tmp + sizeof(tmp) /
              sizeof(int));  // c++11 initialization: costHierarchy{0, 1, 2}
...
struct mapCell_t {
  PointType* xyz;  // it's a pointer to the corresponding point in the point
                   // cloud of submap
